...
class CaissesEvenementView(ctk.CTkFrame):
    ...
    def __init__(self, parent: Any, evenement_id: int | None, callback_refresh=None) -> None:
    ...
    def _build_ui(self) -> None:
        ...
        ctk.CTkButton(top, text="+ Nouvelle caisse", command=self._nouvelle_caisse).pack(side="left")
        ctk.CTkButton(top, text="✏️ Renommer", command=self._renommer_caisse).pack(side="left", padx=(8, 0))
        ctk.CTkButton(
            top,
        ...
        self._tree_debut, self._lbl_total_debut = self._build_section("DÉBUT DE CAISSE", "debut")
        self._tree_fin, self._lbl_total_fin = self._build_section("FIN DE CAISSE", "fin")

        recap = ctk.CTkFrame(self)
        recap.pack(fill="x", padx=8, pady=(4, 8))
        ctk.CTkLabel(recap, text="RÉCAPITULATIF", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10, pady=(8, 4))
        ...
        self._lbl_recap_recette = ctk.CTkLabel(recap, text="RECETTE CAISSE : 0,00 €", font=ctk.CTkFont(weight="bold"))
        self._lbl_recap_recette.pack(anchor="w", padx=10, pady=(2, 8))

    def _build_section(self, titre: str, type_ouverture: str) -> tuple[ttk.Treeview, ctk.CTkLabel]:
        frame = ctk.CTkFrame(self)
        frame.pack(fill="both", expand=True, padx=8, pady=(2, 4))
        ctk.CTkLabel(frame, text=titre, font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10, pady=(8, 4))
        ...
        ctk.CTkButton(actions, text="+ Ajouter ligne", command=lambda t=type_ouverture: self._ajouter_ligne(t)).pack(side="left")
        ctk.CTkButton(actions, text="✏️ Modifier", command=lambda t=type_ouverture: self._modifier_ligne(t)).pack(side="left", padx=(8, 0))
        ctk.CTkButton(
            actions,
            text="🗑️ Supprimer",
        ...
        lbl_total = ctk.CTkLabel(frame, text="TOTAL : 0,00 €", font=ctk.CTkFont(weight="bold"))
    ...
    def refresh(self) -> None:
        ...
        self._lbl_recap_recette.configure(text="RECETTE CAISSE : 0,00 €", text_color=("black", "white"))
        ...
        if self._caisse_id and any(int(c["id"]) == self._caisse_id for c in caisses):
            selected = next(lbl for lbl, cid in self._caisse_options.items() if cid == self._caisse_id)
        else:
            selected = labels[0]
    ...
    def _supprimer_caisse_active(self) -> None:
        if not self._check_caisse():
            return
        if not demander_confirmation(self, "Supprimer", "Supprimer cette caisse et toutes ses lignes ?"):
            return
    ...
    def _modifier_ligne(self, type_ouverture: str) -> None:
        ...
        ligne = {
            "designation": valeurs[0],
            "montant_unitaire": str(valeurs[1]).replace("€", "").replace(" ", "").replace(",", "."),
            "quantite": valeurs[2],
        }
...
class _DialogLigneCaisse(ctk.CTkToplevel):
    ...
    def __init__(self, parent: Any, title: str, ligne: dict | None) -> None:
        ...
        self._designation_var = tk.StringVar(value=(ligne or {}).get("designation") or "")
        self._montant_var = tk.StringVar(value=str((ligne or {}).get("montant_unitaire") or "0"))
        self._quantite_var = tk.StringVar(value=str((ligne or {}).get("quantite") or "1"))
    ...
    def _build(self) -> None:
        ...
        ctk.CTkLabel(self, text="Montant unitaire (€) *").pack(anchor="w", padx=20, pady=(10, 2))
        ...
        ctk.CTkButton(actions, text="Valider", command=self._valider).pack(side="right", padx=(0, 8))
