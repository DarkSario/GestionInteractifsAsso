...
class FicheEvenement(ctk.CTkToplevel):
    ...
    def _build_onglet_general(self, parent: Any) -> None:
        ...
        def row(label: str, widget_fn, **kwargs):
            ...
            ctk.CTkLabel(f, text=label, width=180, anchor="e").pack(side="left", padx=(0, 8))
        ...
        row(
            ...
            lambda p, **kw: ctk.CTkEntry(
                p, textvariable=self._var_date_debut,
                placeholder_text="AAAA-MM-JJ", **kw
            ),
        )
        ...
        row(
            "Date fin",
            lambda p, **kw: ctk.CTkEntry(
                p, textvariable=self._var_date_fin,
                placeholder_text="AAAA-MM-JJ (optionnel)", **kw
            ),
        )
        ...
        row(
            ...
            lambda p, **kw: ctk.CTkOptionMenu(
                p, values=list(LABELS_STATUT.values()),
                variable=self._var_statut, **kw
            ),
        )
        ...
        row(
            "Budget prévisionnel",
            lambda p, **kw: ctk.CTkEntry(
                p, textvariable=self._var_budget,
                placeholder_text="Montant optionnel", **kw
            ),
        )
        ...
        ctk.CTkLabel(
            frame, text="Résumé financier", font=fonts.get("bold")
    ...
    def _sauvegarder_general(self) -> None:
        ...
        if budget_str:
            try:
                ...
                afficher_erreur(self, "Erreur", "Le budget prévisionnel doit être un nombre.")
                return
    ...
    def _build_onglet_billetterie(self, parent: Any) -> None:
        ...
        ctk.CTkButton(
            frame_btn_tarifs, text="+ Ajouter un tarif", width=150,
            command=self._ajouter_tarif,
        ).pack(side="left")
        ctk.CTkButton(
            frame_btn_tarifs, text="✏️ Modifier", width=100,
            command=self._modifier_tarif,
        ).pack(side="left", padx=(6, 0))
        ctk.CTkButton(
            frame_btn_tarifs, text="🗑️ Supprimer", width=110,
        ...
        sv = ttk.Scrollbar(frame_ventes, orient="vertical", command=self._tree_ventes.yview)
        ...
        ctk.CTkButton(
            frame_btn_ventes, text="👁️ Voir détail", width=120,
            command=self._voir_vente,
        ).pack(side="left")
        ctk.CTkButton(
            frame_btn_ventes, text="❌ Annuler vente", width=130,
            fg_color="#dc3545", hover_color="#a71d2a",
        ...
        self._lbl_stats_billet = ctk.CTkLabel(
            frame, text="Total billets : — | Recette brute : — | Net : —",
    ...
    def _nouvelle_vente(self) -> None:
        ...
        tarifs = get_tarifs_evenement(self._evenement_id)
        if not tarifs:
            afficher_erreur(self, "Erreur", "Ajoutez au moins un tarif avant d'enregistrer une vente.")
            return
    ...
    def _enregistrer_vente(self, data: dict) -> None:
        ...
        for ligne in lignes:
            ...
            for _ in range(qte):
                ...
                numero = generer_numero_billet(self._evenement_id, tarif_nom, compteurs[tarif_id])
    ...
    def _build_onglet_depenses(self, parent: Any) -> None:
        ...
        ctk.CTkButton(
            frame_top, text="+ Ajouter une dépense", width=170,
        ...
        sd = ttk.Scrollbar(frame_table, orient="vertical", command=self._tree_depenses.yview)
        ...
        ctk.CTkButton(
            frame_bas, text="✏️ Modifier", width=100,
            command=self._modifier_depense,
        ).pack(side="left")
        ctk.CTkButton(
            frame_bas, text="🗑️ Supprimer", width=110,
            fg_color="#dc3545", hover_color="#a71d2a",
    ...
    def _build_onglet_benevoles(self, parent: Any) -> None:
        ...
        ctk.CTkButton(
            frame_top, text="+ Ajouter un bénévole", width=170,
        ...
        sb = ttk.Scrollbar(frame_table, orient="vertical", command=self._tree_benevoles.yview)
    ...
    def _charger_benevoles(self) -> None:
        ...
        for b in get_benevoles_evenement(self._evenement_id):
            if b.get("membre_id"):
                ...
                nom = f"{b.get('prenom_externe', '')} {b.get('nom_externe', '')}".strip()
                if not nom:
                    nom = "(externe)"
                else:
                    nom += " (ext.)"
...
class _DialogTarif(ctk.CTkToplevel):
    ...
    def __init__(self, parent: Any, tarif: dict | None = None) -> None:
        ...
        self._var_gratuit = tk.BooleanVar(value=bool(tarif["est_gratuit"]) if tarif else False)

        self._build()

    def _build(self) -> None:
        ctk.CTkLabel(self, text="Nom du tarif *").pack(anchor="w", padx=20, pady=(16, 2))
        ...
        ctk.CTkButton(
            frame_btn, text="Annuler", width=100, fg_color="gray",
            hover_color="#555", command=self.destroy
...
class _DialogVente(ctk.CTkToplevel):
    ...
    def _build(self) -> None:
        ...
        def field(label, widget_fn):
            ...
            f.pack(fill="x", pady=3)
            ctk.CTkLabel(f, text=label, width=160, anchor="e").pack(side="left", padx=(0, 8))
            w = widget_fn(f)
            w.pack(side="left")
            return w

        field("Date *", lambda p: ctk.CTkEntry(p, textvariable=self._var_date, width=180))
        field(
            "Canal *",
        ...
        ctk.CTkLabel(self._frame_tireur, text="Nom du tireur", width=160, anchor="e").pack(
            side="left", padx=(0, 8)
        )
        ctk.CTkEntry(self._frame_tireur, textvariable=self._var_nom_tireur, width=180).pack(side="left")
        self._frame_tireur.pack_forget()

        sep = ctk.CTkFrame(scroll, height=2, fg_color="#444")
        sep.pack(fill="x", pady=6)

        ctk.CTkLabel(scroll, text="Tarifs", font=fonts.get("bold")).pack(anchor="w", pady=(0, 4))
        ...
        for tarif in self._tarifs:
            ...
            ctk.CTkLabel(f, text=f"{tarif['nom']}  ({tarif['prix']:.2f} €)", width=220, anchor="w").pack(
                side="left"
            )
    ...
    def _actualiser_total(self, *_) -> None:
        ...
        self._lbl_total.configure(text=f"Montant total : {total:,.2f} €".replace(",", " ").replace(".", ","))
        if mode == "sumup":
            self._lbl_frais.configure(
                text=f"Frais SumUp ({taux}%) : {frais:,.2f} €".replace(",", " ").replace(".", ",")
            )
        else:
...
class _DialogDepense(ctk.CTkToplevel):
    ...
    def __init__(self, parent: Any, depense: dict | None = None) -> None:
        ...
        self._var_categorie = tk.StringVar(value=depense.get("categorie") or "" if depense else "")
        self._var_mode = tk.StringVar(
            value=MODES_DEPENSE.get(depense.get("mode_paiement") or "", "—") if depense else "—"
        )
        self._var_commentaire = tk.StringVar(value=depense.get("commentaire") or "" if depense else "")
        self._fournisseurs: list[dict] = []
        self._fournisseur_id: int | None = depense.get("fournisseur_id") if depense else None
    ...
    def _build(self) -> None:
        ...
        def field(label, widget_fn):
            f = ctk.CTkFrame(scroll, fg_color="transparent")
            f.pack(fill="x", pady=3)
            ctk.CTkLabel(f, text=label, width=140, anchor="e").pack(side="left", padx=(0, 8))
            w = widget_fn(f)
            w.pack(side="left")
            return w

        field("Libellé *", lambda p: ctk.CTkEntry(p, textvariable=self._var_libelle, width=240))
        field("Montant *", lambda p: ctk.CTkEntry(p, textvariable=self._var_montant, width=240))
        field(
            "Date *",
            lambda p: ctk.CTkEntry(p, textvariable=self._var_date, width=240, placeholder_text="AAAA-MM-JJ"),
        )
        field("Catégorie", lambda p: ctk.CTkEntry(p, textvariable=self._var_categorie, width=240))

        # Fournisseur
        f_four = ctk.CTkFrame(scroll, fg_color="transparent")
        f_four.pack(fill="x", pady=3)
        ctk.CTkLabel(f_four, text="Fournisseur", width=140, anchor="e").pack(side="left", padx=(0, 8))
        ...
        if self._fournisseur_id:
            four = next((f for f in self._fournisseurs if f["id"] == self._fournisseur_id), None)
        ...
        field(
            "Mode paiement",
            lambda p: ctk.CTkOptionMenu(
                p, values=list(MODES_DEPENSE.values()), variable=self._var_mode, width=240
            ),
        )
        field("Commentaire", lambda p: ctk.CTkEntry(p, textvariable=self._var_commentaire, width=240))
...
class _DialogBenevole(ctk.CTkToplevel):
    ...
    def __init__(
        ...
        self._var_nom_ext = tk.StringVar(value=benevole.get("nom_externe") or "" if benevole else "")
        self._var_prenom_ext = tk.StringVar(value=benevole.get("prenom_externe") or "" if benevole else "")
        self._var_role = tk.StringVar(value=benevole.get("role") or "" if benevole else "")
        self._var_h_debut = tk.StringVar(value=benevole.get("heure_debut") or "" if benevole else "")
        self._var_h_fin = tk.StringVar(value=benevole.get("heure_fin") or "" if benevole else "")
        self._var_statut = tk.StringVar(
            value=STATUTS_BENEVOLE.get(benevole.get("statut", "confirme"), "Confirmé") if benevole else "Confirmé"
        )
        self._var_commentaire = tk.StringVar(value=benevole.get("commentaire") or "" if benevole else "")
    ...
    def _build(self) -> None:
        ...
        ctk.CTkLabel(f_type, text="Type", width=130, anchor="e").pack(side="left", padx=(0, 8))
        ctk.CTkRadioButton(
            f_type, text="Membre", variable=self._var_type, value="Membre",
            command=self._on_type_change
        ).pack(side="left", padx=(0, 12))
        ctk.CTkRadioButton(
            f_type, text="Externe", variable=self._var_type, value="Externe",
            command=self._on_type_change
        ).pack(side="left")
        ...
        ctk.CTkLabel(f_prenom, text="Prénom", width=130, anchor="e").pack(side="left", padx=(0, 8))
        ctk.CTkEntry(f_prenom, textvariable=self._var_prenom_ext, width=240).pack(side="left")

        f_nom = ctk.CTkFrame(self._frame_externe, fg_color="transparent")
        f_nom.pack(fill="x", pady=2)
        ctk.CTkLabel(f_nom, text="Nom", width=130, anchor="e").pack(side="left", padx=(0, 8))
        ctk.CTkEntry(f_nom, textvariable=self._var_nom_ext, width=240).pack(side="left")

        def field(label, widget_fn):
            f = ctk.CTkFrame(scroll, fg_color="transparent")
            f.pack(fill="x", pady=3)
            ctk.CTkLabel(f, text=label, width=130, anchor="e").pack(side="left", padx=(0, 8))
            w = widget_fn(f)
            w.pack(side="left")
            return w
        ...
        field(
            "Heure début",
            lambda p: ctk.CTkEntry(p, textvariable=self._var_h_debut, width=100, placeholder_text="HH:MM"),
        )
        field(
            "Heure fin",
            lambda p: ctk.CTkEntry(p, textvariable=self._var_h_fin, width=100, placeholder_text="HH:MM"),
        )
        field(
            "Statut",
            lambda p: ctk.CTkOptionMenu(
                p, values=list(STATUTS_BENEVOLE.values()),
                variable=self._var_statut, width=180
            ),
        )
        field("Commentaire", lambda p: ctk.CTkEntry(p, textvariable=self._var_commentaire, width=240))
    ...
    def _on_type_change(self) -> None:
        if self._var_type.get() == "Membre":
            self._frame_membre.pack(fill="x", pady=3, after=self._frame_membre.master.winfo_children()[1])
            self._frame_externe.pack_forget()
        else:
            self._frame_membre.pack_forget()
            self._frame_externe.pack(fill="x", pady=3, after=self._frame_externe.master.winfo_children()[1])
...
class _DialogMotif(ctk.CTkToplevel):
    ...
    def __init__(self, parent: Any) -> None:
        ...
        ctk.CTkLabel(self, text="Motif (optionnel) :").pack(anchor="w", padx=20, pady=(16, 4))
